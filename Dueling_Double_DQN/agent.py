...
writer = SummaryWriter(log_dir=f"runs/atari_Dueling_Double_dqn_{int(time.time())}")
...
class DuelingDoubleDQN:
        ...
        while 1:
            ...
            while not done:
                ...
                if self.total_step % 100000 == 0:
                    torch.save(self.Q.state_dict(), './checkpoints/model_state_dict_1.pth')
    ...
    def train_step(self):
        ...
        with torch.no_grad():
            ...
            q_next = torch.gather(self.targetQ(next_states), dim=1, index=next_actions)
